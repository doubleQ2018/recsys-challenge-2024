...
class BaseModel(nn.Module):
    ...
    def fit_only_train(self, data_generator, epochs=1, max_gradient_norm=10., **kwargs):
        self._max_gradient_norm = max_gradient_norm
        self._best_metric = np.Inf if self._monitor_mode == "min" else -np.Inf
        self._stopping_steps = 0
        self._steps_per_epoch = len(data_generator)
        ...
        self._epoch_index = 0
        if self._eval_steps is None:
            self._eval_steps = self._steps_per_epoch * 10
        for epoch in range(epochs):
            logging.info(f"************ Epoch={epoch} start ************")
            self._epoch_index = epoch
            self.train_epoch(data_generator)
            self.save_weights(self.checkpoint)
            logging.info("Saved model: {}".format(self.checkpoint))
        logging.info("Training finished.")
